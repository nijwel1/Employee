@extends('layouts.app')
@section('title', '| Designation Trash')
@section('header', 'Designation Trash')
@section('content')
    @push('css')
    @endpush
                    ...
                    <div class="col-lg-6">
                        <div class="d-flex justify-content-end mb-3">
                            <button type="button" id="deleteAllBtn" disabled class="btn btn-sm btn-danger me-2"> <i
                                ...
                                Delete</button>
                            <a href="{{ route('designation.index') }}" class="btn btn-sm btn-base"><i
                        ...
                        <table id="" class="table table-hover table-responsive">
                            ...
                            <tbody>
                                @forelse ($designations as $key => $designation)
                                    <tr>
                                        <td width="5%">
                                            <form id="deleteAllForm" action="{{ route('designation.delete.all') }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <input type="checkbox" name="id[]" value="{{ $designation->id }}">
                                                {{ $key + 1 }}
                                            </form>
                                        </td>
                                        ...
                                        <td width="15%">
                                            <span
                                                class="badge rounded-pill @if ($designation->status == 'active') text-bg-success @else text-bg-danger @endif ">{{ $designation->status }}</span>
                                        </td>
                                        <td width="10%">
                                            <div class="d-flex gap-1">
                                                <form action="{{ route('designation.restore', $designation->id) }}"
                                                    method="get">
                                                ...
                                                <form action="{{ route('designation.forceDelete', $designation->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
