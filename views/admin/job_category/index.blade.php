                    ...
                    <div class="col-lg-6">
                        <div class="">
                            <div class="d-flex justify-content-end mb-3">
                                <a href="{{ route('job.category.trash') }}" class="btn btn-danger btn-sm me-2"> <i
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <table id="myTable" class="display">
                                ...
                                <tbody>
                                    @forelse ($job_categories as $key => $job_Category)
                                        <tr>
                                            <td width="5%">{{ $key + 1 }}</td>
                                            <td>{{ $job_Category->title }}</td>
                                            <td>{{ $job_Category->remarks }}</td>
                                            <td width="15%">
                                                <span
                                                    class="badge rounded-pill @if ($job_Category->status == 'active') text-bg-success @else text-bg-danger @endif ">{{ $job_Category->status }}</span>
                                            </td>
                                            <td width="10%">
                                                <div class="d-flex gap-1">
                                                    <button class="btn btn-outline-base btn-sm edit"
                                                        data-id="{{ $job_Category->id }}" data-bs-toggle="modal"
                                                        data-bs-target="#editModal">
                                                        <i class="far fa-edit"></i>
                                                    </button>
                                                    <form action="{{ route('job.category.destroy', $job_Category->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button Category="submit" id="delete"
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{ route('job.category.store') }}" method="POST">
                                            @csrf
                                            <div class="mb-3">
                                                <label for="input1" class="form-label">Tile</label>
                                                <input Category="text" name="title" class="form-control" id="input1"
                                            ...
                                            <div class="modal-footer">
                                                <button Category="submit" class="btn btn-primary">Submit</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
    ...
    @push('js')
        <script>
            $('body').on('click', '.edit', function() {
                var id = $(this).data('id');
                $.get("{{ url('admin/job-category/edit') }}/" + id,
                    function(data) {
